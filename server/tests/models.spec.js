...
describe('Entry model', () => {
    ...
    it('should throw an error if no entry (findById)', () => {
        function testError() {
            const entry = Entry.findById(1000);
        }
        expect(testError).toThrowError('That entry does not exist.');
    });
    ...
    it('should throw an error if no entry (findByTitle)', () => {
        const title = "Nothing"
        function testError() {
            Entry.findByTitle(title);
        }
        expect(testError).toThrowError('That entry does not exist.');
    });

    it('should create a new entry', () => {
        const newEntry = new Entry(testEntry);
        expect(newEntry).toEqual({...newEntry});
    });
